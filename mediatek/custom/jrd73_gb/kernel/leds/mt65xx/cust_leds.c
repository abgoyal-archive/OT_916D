...
/*add for virtual_key_backlight*/
static   volatile  unsigned int key_mapped_level_before = 0;
unsigned int Key_SetBacklight(int level,int drv)
{

	mt_set_gpio_mode(GPIO109, 0);
	...
	unsigned int i;
   	...
   	unsigned int mapped_level;
        ...
        mapped_level = level/16;
	if(key_mapped_level_before == mapped_level)
		return;
	if(mapped_level == 0)
	{
	...
	else if(mapped_level < key_mapped_level_before)
	{
	
		mt_set_gpio_out(GPIO109, 1);
	
		udelay(40);	
		for(i = 0;i<(key_mapped_level_before - mapped_level);i++)
		{
	...
	key_mapped_level_before = mapped_level;
...
static struct cust_mt65xx_led cust_led_list[MT65XX_LED_TYPE_TOTAL] = {
	...
	{"button-backlight",  MT65XX_LED_MODE_CUST, (int)Key_SetBacklight},
	{"keyboard-backlight",MT65XX_LED_MODE_PWM, PWM7},
