...
/*melfas touch panel firmware upgrade... wangdongliang*/


//step 1:get firmware version now

	unsigned char id_bufff = 0x21;
	i2c_client->addr = 0x40& I2C_MASK_FLAG;
	i2c_master_send(i2c_client, &id_bufff, 1);
	i2c_master_recv(i2c_client, &id_bufff, 1);
	printk("*melfas touch panel firmware is %d*\n", id_bufff);

//step 2:if firmware version isn't 0x06 we will upgrade the firmware (V06)
	if(id_bufff != 0x06)
	{
...
//step 3:if upgraded we should get the new firmware version
		id_bufff = 0x21;
		i2c_client->addr = 0x40& I2C_MASK_FLAG;	//reset device's addr to normal mode
		i2c_master_send(i2c_client, &id_bufff, 1);
		i2c_master_recv(i2c_client, &id_bufff, 1);
		printk(" %d  new firmware is %d\n", i, id_bufff);
	}

/*melfas touch panel end firmware upgrade... wangdongliang*/
	...
	mt65xx_eint_set_sens(0, CUST_EINT_TOUCH_PANEL_SENSITIVE);
	mt65xx_eint_set_hw_debounce(0, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);
	mt65xx_eint_registration(0, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_TOUCH_PANEL_POLARITY, tpd_eint_interrupt_handler, 1);
