...
//
//============================================================
unsigned char nBinary[] = {
#include "MTH_PISCO_R01_V06_bin.h"
};

//---------------------------------
//	Downloading functions
//---------------------------------
int  mcsdl_download(unsigned char *pData, UINT16 nLength);

int  mcsdl_enter_download_mode(void);
void mcsdl_write_download_mode_signal(void);

int  mcsdl_i2c_erase_flash(void);
int  mcsdl_i2c_prepare_erase_flash(void);
int  mcsdl_i2c_read_flash(UINT8 *pBuffer, UINT16 nAddr_start, UINT8 cLength);
int  mcsdl_i2c_prepare_program(void);
int  mcsdl_i2c_program_flash( UINT8 *pData, UINT16 nAddr_start, UINT8 cLength );


//---------------------------------
//	I2C Functions
//---------------------------------
unsigned char melfas_i2c_read( UINT8 slave_addr, UINT8 *pData, UINT8 cLength);
unsigned char melfas_i2c_write(UINT8 slave_addr, UINT8 *pData, UINT8 cLength);
...
void mcsdl_print_result(int nRet);
#endif
...
int mcsdl_download_binary_data(void)
{
	...
	udelay(MCSDL_DELAY_100US);

	#endif
	unsigned char* PnBinary = nBinary;
 //  	 mt65xx_eint_mask(0);							// Disable Baseband touch interrupt ISR.
...
//	MELFAS_DISABLE_WATCHDOG_TIMER_RESET();			// Disable Baseband watchdog timer
	...
	mt_set_gpio_mode(GPIO187, 0);
	mt_set_gpio_mode(181, 0);
	mt_set_gpio_mode(182, 0);

 
	ret = mcsdl_download( (unsigned char*) PnBinary, (UINT16)sizeof(nBinary));

	mt_set_gpio_mode(GPIO187, GPIO_CTP_EINT_PIN_M_EINT);
	mt_set_gpio_dir(GPIO187, GPIO_DIR_IN);
...
//	mt65xx_eint_unmask(0);								
	#if MELFAS_ENABLE_DBG_PRINT						// Roll-back Baseband touch interrupt ISR.
...
int mcsdl_download_binary_file(void)
{
	...
	//==================================================
		...
		//------------------------------
            ...
            //sunshine need to define Download filename
...
int mcsdl_download(unsigned char *pData, UINT16 nLength )
{
	...
	printk("step 1 melfas touch panel enter download mode...\n");
	#endif
	...
	udelay(MCSDL_DELAY_1MS);					// Delay '1 msec'
	...
	#if MELFAS_ENABLE_DBG_PROGRESS_PRINT
	printk("step 2  melfas touch panel Erasing...\n");
	#endif
	...
	//--------------------------------------------------------------
	#if MELFAS_ENABLE_DBG_PROGRESS_PRINT
	printk("step 3 melfas touch panel Verify Erasing... \n");
	#endif
	...
	//--------------------------------------------------------------
	#if MELFAS_ENABLE_DBG_PROGRESS_PRINT
	printk("step 4 melfas touch panel Preparing Program... \n");
	#endif
   ...
   //
	...
	#if MELFAS_ENABLE_DBG_PROGRESS_PRINT
	printk("step 5 melfas touch panel Program flash... \n ");
	#endif
	...
	for( nStart_address = 0; nStart_address < nLength; nStart_address+=cLength ){
		...
		udelay(MCSDL_DELAY_500US);					// Delay '500 usec'
	...
	#if MELFAS_ENABLE_DBG_PROGRESS_PRINT
	printk("\n");
	printk("step 6 melfas touch panel Verify flash...  \n");
	#endif
...
MCSDL_DOWNLOAD_FINISH :
	...
	melfas_i2c_write( MCSDL_I2C_SLAVE_ADDR_ORG, buffer, 1 );

	TKEY_INTR_SET_INPUT();									// Rollback Interrupt port
	mdelay(180);
//    udelay(MCSDL_DELAY_45MS);							// Delay about '200 msec'
//    udelay(MCSDL_DELAY_45MS);
//    udelay(MCSDL_DELAY_45MS);
//    udelay(MCSDL_DELAY_45MS);
...
int mcsdl_enter_download_mode(void)
{
	...
	mdelay(90);
//	udelay(MCSDL_DELAY_45MS);						// Delay for Stable VDD
//	udelay(MCSDL_DELAY_45MS);
	...
	mdelay(25);
//	udelay(MCSDL_DELAY_25MS); 						// Delay '25 msec'
	...
	mdelay(2);
//	udelay(MCSDL_DELAY_1MS); 						// Delay '2 msec'
//	udelay(MCSDL_DELAY_1MS);
	...
	bRet = melfas_i2c_read( MCSDL_I2C_SLAVE_ADDR_ORG, &cData, 1 );
...
void mcsdl_write_download_mode_signal(void)
{
	...
	for(i=0; i<14; i++){
		...
		TKEY_I2C_SCL_SET_HIGH();	udelay(MCSDL_DELAY_15US);
...
int mcsdl_i2c_prepare_erase_flash(void)
{
   ...
   for(i=0; i<4; i++){
		...
		bRet = melfas_i2c_write(MCSDL_I2C_SLAVE_ADDR_ORG, &i2c_buffer[i], 1 );
   ...
   //-----------------------------
	...
	udelay(MCSDL_DELAY_500US);                  		// Delay 500usec

	bRet = melfas_i2c_read(MCSDL_I2C_SLAVE_ADDR_ORG, &ucTemp, 1 );
...
int mcsdl_i2c_erase_flash(void)
{
   ...
   //-----------------------------
	mdelay(45);
//	udelay(MCSDL_DELAY_45MS);                  			// Delay 45ms
...
int mcsdl_i2c_read_flash( UINT8 *pBuffer, UINT16 nAddr_start, UINT8 cLength)
{
	...
	for(i=0; i<4; i++){
		...
		bRet = melfas_i2c_write( MCSDL_I2C_SLAVE_ADDR_ORG, &cmd[i], 1 );
	...
	for(i=0; i<(int)cmd[3]; i++){
		...
		udelay(MCSDL_DELAY_100US);                  // Delay about 100us

		bRet = melfas_i2c_read( MCSDL_I2C_SLAVE_ADDR_ORG, pBuffer++, 1 );
...
int mcsdl_i2c_prepare_program(void)
{
	...
	udelay(MCSDL_DELAY_500US);                     // delay about  500us
	...
	//Cowon 에서의 Download문제 발생으로 확인필요.
	    bRet = melfas_i2c_read( MCSDL_I2C_SLAVE_ADDR_ORG, i2c_buffer, 1 );

    if( bRet == FALSE || i2c_buffer[0] != MCSDL_I2C_ACK_PREPARE_PROGRAM)
        goto MCSDL_I2C_PREPARE_PROGRAM_FINISH;

    */
	bRet = melfas_i2c_read( MCSDL_I2C_SLAVE_ADDR_ORG, &i2c_buffer[4], 1 );
...
int mcsdl_i2c_program_flash( UINT8 *pData, UINT16 nAddr_start, UINT8 cLength )
{
	...
	udelay(MCSDL_DELAY_150US);                  // Delay about 150us

	for(i=0; i<(int)cmd[3]; i+=2){

		bRet = melfas_i2c_write( MCSDL_I2C_SLAVE_ADDR_ORG, &pData[i+1], 1 );

		if( bRet == FALSE )
			goto MCSDL_I2C_PROGRAM_FLASH_FINISH;

		udelay(MCSDL_DELAY_100US);                  // Delay about 150us

		bRet = melfas_i2c_write( MCSDL_I2C_SLAVE_ADDR_ORG, &pData[i], 1 );
...
//
//		melfas_i2c_read(), melfas_i2c_write() must return
//			TRUE (1) if success,
//			FALSE(0) if failed.
//
...
unsigned char melfas_i2c_read( UINT8 slave_addr, UINT8 *pData, UINT8 cLength)
{
	unsigned char bRet;
	int ret;
	i2c_client->addr = slave_addr& I2C_MASK_FLAG;
	
	ret = i2c_master_recv(i2c_client, (const char*)pData, cLength);
	if(ret < 0)
	{
		bRet = FALSE;
		printk("xxxxxxxxx updata failed read xxxxxxxx\n");
	}
	else
		bRet = TRUE;
...
unsigned char melfas_i2c_write(UINT8 slave_addr, UINT8 *pData, UINT8 cLength)
{
	...
	ret = i2c_master_send(i2c_client, (const char*)pData, cLength);
	if(ret < 0)
	{
		bRet = FALSE;
		printk("xxxxxxxxx updata failed write xxxxxxxx\n");
	}
	else
		bRet = TRUE;
